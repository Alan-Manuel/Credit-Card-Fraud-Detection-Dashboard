...
- Each transaction includes details such as **amount, merchant, and type**.  
- Fraudulent transactions are rare (~1%), so we use **SMOTE** to balance the dataset.  

👉 Use the sidebar to upload a file or analyze the default dataset!
""")
...
if uploaded_file is not None:
    ...
    DATA_PATH = "credit_card_fraud_dataset.csv"  # 👈 Replace with your renamed file
    data = pd.read_csv(DATA_PATH)

# Check for required column
if "IsFraud" not in data.columns:
    st.error("⚠️ The dataset must contain a column named 'IsFraud'.")
    st.stop()
...
if X.shape[1] == 0:
    st.error("⚠️ No numeric features found. Please choose numeric columns.")
    st.stop()
